...
GLOBAL_INDICATOR_REGISTRY = {
    ...
    "day_of_week": {
        ...
        "n_params": 0,
        "description": (
            "Day of Week (no parameters)\n"
            "\n"
            "Weekday of each trading bar: 0=Monday ... 4=Friday.\n"
            "Captures systematic intraday closing bias by day of week\n"
            "(e.g. risk-off into weekend may lower S on Fridays).\n"
            "Implemented as a global indicator so C++ receives it via\n"
            "the global_x injection mechanism, same as market_mean_S."
        ),
    },
}
...
def _compute_daily_RMS(close_matrix, high_matrix, low_matrix):
    ...
    S = np.where(np.isfinite(S) & (np.abs(S) <= 1.05), S, np.nan)
...
def global_day_of_week(close_matrix, high_matrix, low_matrix, params, dates=None):
    """
    Weekday series [n_days]: 0=Mon, 1=Tue, 2=Wed, 3=Thu, 4=Fri.
    Requires dates array of length n_days.
    Falls back to NaN series if dates not provided.
    """
    import datetime
    n_days = close_matrix.shape[1]
...
GLOBAL_INDICATOR_REGISTRY["day_of_week"]["func"]           = global_day_of_week
...
class GlobalIndicator:
    ...
    def __init__(self, name: str, params: list,
        ...
        if "dates" in inspect.signature(func).parameters:
    ...
    def compute(self, high, low, close, dates=None):
        ...
        dates is accepted but ignored — the series was precomputed at init.
        """
        n = len(close)
